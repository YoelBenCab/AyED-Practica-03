...
// FASE II
 sparse_vector_t::sparse_vector_t(const vector_t<double>& v, const double eps)
    : pv_(), nz_(0), n_(0) {
  n_ = v.get_size();
  int size_sparse_vector{0};
  for (int i = 0; i < v.get_size(); ++i) { // contar cuantos elementos son distintos de cero
    if (!IsNotZero(v[i])) {
      continue;
    }
    ++size_sparse_vector;
  }
  nz_ = size_sparse_vector;
  pv_.resize(size_sparse_vector);
  for (int i = 0, j = 0; i < v.get_size(); ++i) {
    // comprobar que el valor es dintito de cero
    if (!IsNotZero(v[i])) {
      continue; // si v[i] es igual a cero, salta la iteracion y no almacena el valor
    }
    // almacenar el valor y el indice en un objeto pair_t<double> = pair_double_t
    pair_double_t value(v[i], i);
    // meter el objeto pair_double_t en un objeto vector_t<pair_double_t> = pair_vector_t
    pv_[j] = value;
    ++j;
  }
}
